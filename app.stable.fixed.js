...
const DOWNLOAD_META = {
  'genes.json': { label: 'Search JSON', description: 'Main gene dataset used by the static site.' },
  'Amorphophallus_konjac.clean.gff': { label: 'GFF annotation', description: 'Gene models, coordinates and transcript structure.' },
  'Amorphophallus_konjac.clean.cds': { label: 'CDS FASTA', description: 'Coding sequences for local analysis and export.' },
  'Amorphophallus_konjac.clean.pep': { label: 'Protein FASTA', description: 'Protein sequences for annotation and domain analysis.' },
  'zen_annotation_v2.0.tsv': { label: 'ZEN annotation', description: 'Homologs, KO/EC and species-origin annotations.' },
  'zen_go_v2.0.tsv': { label: 'ZEN GO', description: 'GO annotation results.' },
  'zen_goslim_v2.0.tsv': { label: 'ZEN GO slim', description: 'GO slim categorization results.' },
  'zen_interpro_v2.0.tsv': { label: 'ZEN InterPro', description: 'InterPro domain annotation results.' },
  'zen_pfam_v2.0.tsv': { label: 'ZEN Pfam', description: 'Pfam domain annotation results.' }
};
...
const state = {
  ...
  sequenceCache: { cds: null, protein: null }
};
...
function renderSearchStatus({ query = '', count = null, elapsedMs = null, filters = [] } = {}) {
  ...
  box.hidden = false;
  const parts = [];
  if (query) parts.push(`Query: ${escapeHtml(query)}`);
  if (count !== null && count !== undefined) parts.push(`Results: ${formatNumber(count)}`);
  if (elapsedMs !== null && elapsedMs !== undefined) parts.push(`Time: ${escapeHtml(formatDuration(elapsedMs))}`);
  if (filters && filters.length) parts.push(`Filters: ${filters.map(escapeHtml).join(' 路 ')}`);
...
function renderHomeModules() {
  const items = [
    ['基因查询', '#search', '输入 Gene ID、GO、KEGG、Pfam 或转录因子家族进行查询。'],
    ['魔芋研究专题', '#topics', '进入 KGM、CSLA、转录因子、抗病和细胞壁专题入口。'],
    ['注释分类浏览', '#browse', '按 GO、InterPro、Pfam 和同源物种快速浏览。'],
    ['数据下载', '#downloads', '下载整合表、GFF、CDS、protein FASTA 和 ZEN 文件。'],
    ['数据来源与引用', '#sources', '查看数据来源、引用格式、许可说明和维护信息。'],
    ['使用帮助', '#help', '查看搜索示例、本地运行方法和后续升级计划。']
  ];
  qs('homeModules').innerHTML = items.map(([title, href, desc]) => `
  ...
  qs('detailContent').innerHTML = `
    <div class="detail-grid">
      <article class="card">
        <h3>当前数据集</h3>
        <dl>
          <dt>Species</dt><dd>Amorphophallus konjac</dd>
          <dt>Genome</dt><dd>PlantGARDEN t78372.G001 / NCBI assembly GCA_022559845.1</dd>
          <dt>Gene models</dt><dd>PlantGARDEN clean.gff</dd>
          <dt>CDS / protein</dt><dd>clean.cds / clean.pep</dd>
          <dt>Functional annotation</dt><dd>ZEN annotation v2.0, GO, GO slim, KEGG/KO/EC, InterPro, Pfam, PlantTFDB</dd>
          <dt>Website version</dt><dd>v1.2.1 static build</dd>
          <dt>Updated</dt><dd>${escapeHtml(state.summary.website_last_updated || '待补充')}</dd>
        </dl>
      </article>
      <article class="card">
        <h3>引用与许可</h3>
        <dl>
          <dt>PlantGARDEN</dt><dd>待补充</dd>
          <dt>ZEN</dt><dd>待补充</dd>
          <dt>NCBI assembly</dt><dd>待补充</dd>
          <dt>External resources</dt><dd>GO, KEGG, InterPro, Pfam, PlantTFDB</dd>
          <dt>Citation</dt><dd>待补充</dd>
          <dt>License</dt><dd>待补充</dd>
        </dl>
      </article>
      <article class="card full">
        ...
        <ul class="check-list">
          ...
          <li>带有“待补充”标记的来源、引用和许可信息，发布前需要人工核对。</li>
...
function parseFastaToMap(text) {
  ...
  const save = () => {
    if (!currentId) return;
    map.set(currentId, parts.join('').replace(/\s+/g, ''));
  };
...
async function loadSequenceMap(type) {
  if (state.sequenceCache[type]) return state.sequenceCache[type];
  const res = await fetch(SEQUENCE_FILES[type]);
  if (!res.ok) throw new Error(`Unable to load sequence file: ${res.status}`);
  const text = await res.text();
  state.sequenceCache[type] = parseFastaToMap(text);
  return state.sequenceCache[type];
}

async function getGeneSequence(geneId, type) {
  const map = await loadSequenceMap(type);
...
async function renderSequenceViewer(geneId, type) {
  ...
  try {
    const seq = await getGeneSequence(geneId, type);
    if (!seq) {
      viewer.innerHTML = `<p class="muted">当前基因没有提供 ${label} 序列：<code>${escapeHtml(geneId)}</code>。</p><p class="muted">可以尝试另一种序列类型，或查看下载文件中的完整 FASTA 导出。</p>`;
      return;
    }
    state.currentSequence = { geneId, type, seq };
    const header = `>${geneId}|${label}|length=${seq.length}`;
    viewer.innerHTML = `
      <div class="sequence-toolbar">
        <strong>${label} 序列</strong>
        <span class="muted">长度：${formatNumber(seq.length)} ${type === 'protein' ? 'aa' : 'bp'}</span>
        <div class="sequence-actions">
          <button class="mini-link" data-action="copy-sequence">复制序列</button>
          <button class="mini-link" data-action="download-sequence">下载 FASTA</button>
        </div>
      </div>
      <pre class="sequence-box" id="sequenceText">${escapeHtml(header + '\n' + wrapSequence(seq))}</pre>
    `;
  } catch (error) {
    state.currentSequence = null;
    viewer.innerHTML = `<p class="muted">序列加载失败：${escapeHtml(error.message)}</p>`;
  }
}
...
function downloadCurrentSequence() {
  ...
  const { geneId, type, seq } = state.currentSequence;
  const text = `>${geneId}|${sequenceLabel(type)}|length=${seq.length}\n${wrapSequence(seq)}\n`;
...
function renderGeneDetail(gene) {
  qs('detailTitle').textContent = `${gene.gene_id}${gene.gene_symbol ? ' / ' + gene.gene_symbol : ''}`;
  const location = geneLocation(gene);
  qs('detailContent').innerHTML = `
    <div class="detail-grid">
      ${renderDetailCard('Basic information', [
        ['Gene ID', `<code>${escapeHtml(gene.gene_id)}</code> <button class="mini-link" data-action="copy-gene-id" data-gene-id="${escapeHtml(gene.gene_id)}" data-copy="${escapeHtml(gene.gene_id)}" data-copy-message="已复制 Gene ID">Copy Gene ID</button>`],
        ['Symbol', renderFallback(gene.gene_symbol)],
        ['Aliases', renderTagList(gene.aliases, 12)],
        ['Species', renderFallback(gene.species)],
        ['Transcript / Protein', `${renderFallback(gene.transcript_id)} / ${renderFallback(gene.protein_id)}`]
      ])}
      ${renderDetailCard('Gene location', [
        ['Chromosome / contig', renderFallback(gene.chromosome)],
        ['Coordinates', `<code>${escapeHtml(location)}</code> <button class="mini-link" data-copy="${escapeHtml(location)}" data-copy-message="已复制坐标">Copy location</button>`],
        ['Strand', renderFallback(gene.strand)],
        ['CDS length', `${formatNumber(gene.cds_length)} bp`],
        ['Protein length', `${formatNumber(gene.protein_length)} aa`],
        ['CDS / exon count', `${formatNumber(gene.cds_count)} / ${formatNumber(gene.exon_count)}`],
        ['Genome browser locus', renderFallback(gene.genome_browser_locus)]
      ])}
      ${renderDetailCard('Functional summary', [
        ['Annotation', renderFallback(gene.functional_annotation)],
        ...
        ['Source', renderFallback(gene.source)],
        ['Data status', renderFallback(gene.data_status)],
        ['Notes', renderFallback(gene.notes)],
        ['Updated', renderFallback(gene.last_updated)]
      ])}
      ${renderDetailCard('GO annotation', [
      ...
      ${renderDetailCard('Transcription factor', [
        ['PlantTFDB family', renderFallback(gene.plantTFDB_family)],
        ['Description', renderFallback(gene.plantTFDB_description)]
      ])}
      ${renderDetailCard('Homolog hits', [
        ['Target species', renderFallback(gene.target_species)],
        ['Target family', renderFallback(gene.target_family)],
      ...
      ${renderDetailCard('Sequences & download', [
        ['Sequence viewer', `
          <div class="sequence-actions">
            <button class="button ghost small" data-action="show-sequence" data-sequence-type="cds" data-gene-id="${escapeHtml(gene.gene_id)}">View CDS</button>
            <button class="button ghost small" data-action="show-sequence" data-sequence-type="protein" data-gene-id="${escapeHtml(gene.gene_id)}">View protein</button>
          </div>
...
function downloadLinks() {
  return `
    <p class="muted">Sequence files and annotation exports are available below.</p>
    <p class="download-inline">
      <a href="./downloads/Amorphophallus_konjac.clean.cds" download>CDS FASTA</a>
      <a href="./downloads/Amorphophallus_konjac.clean.pep" download>Protein FASTA</a>
      <a href="./downloads/Amorphophallus_konjac.clean.gff" download>GFF</a>
    </p>
  `;
}
...
function renderResults(rows) {
  ...
  if (!rows.length) {
    ...
    qs('resultCount').textContent = '0 results';
    ...
    tr.innerHTML = `
      <td data-label="Gene ID">
        ...
        <div class="row-actions">
          <button class="mini-link" data-action="copy-gene-id" data-gene-id="${escapeHtml(gene.gene_id)}" data-copy="${escapeHtml(gene.gene_id)}" data-copy-message="已复制 Gene ID">Copy ID</button>
          <button class="mini-link" data-search="${escapeHtml(gene.gene_id)}">Search</button>
        </div>
      </td>
      ...
      <td data-label="Action"><button class="button ghost small" data-view="${escapeHtml(gene.gene_id)}">View details</button></td>
    `;
    fragment.appendChild(tr);
  });
  tbody.appendChild(fragment);
  qs('resultsPanel').hidden = false;
  qs('resultCount').textContent = `${rows.length.toLocaleString()} results`;
...
function buildSearchChips(totalRows, totalPages) {
  ...
  const sortMode = qs('sortMode')?.value || 'relevance';
  const pills = [`Results: ${formatNumber(totalRows)}`, totalPages ? `Page ${state.currentPage}/${totalPages}` : 'Page 0/0'];
  if (query) pills.push(`Query: ${query}`);
  if (topic) pills.push(`Topic: ${topic.title}`);
  if (fieldMode !== 'all') pills.push(`Field: ${qs('fieldFilter')?.selectedOptions?.[0]?.textContent || fieldMode}`);
  if (species) pills.push(`Species: ${species}`);
  if (annotation) pills.push(`Annotation: ${qs('annotationFilter')?.selectedOptions?.[0]?.textContent || annotation}`);
  if (sortMode !== 'relevance') pills.push(`Sort: ${qs('sortMode')?.selectedOptions?.[0]?.textContent || sortMode}`);
  pills.push(`Page size: ${qs('pageSize')?.selectedOptions?.[0]?.textContent || '100'}`);
  return pills;
}
...
function renderQueryHint() {
  ...
  box.hidden = false;
  box.innerHTML = `<strong>Related terms</strong><div class="tag-list">${terms.map(t => `<button class="tag tag-button" data-search="${escapeHtml(t)}">${escapeHtml(t)}</button>`).join('')}</div>`;
}
...
function renderActiveTopic() {
  ...
  box.hidden = false;
  box.innerHTML = `
    <strong>Active topic: ${escapeHtml(topic.title)}</strong>
    <span class="muted">${escapeHtml(topic.description)}</span>
    <button class="mini-link" id="clearTopic">Clear topic</button>
  `;
...
function bindEvents() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (seqCopy) {
      if (!state.currentSequence) { showToast('Please view a sequence first'); return; }
